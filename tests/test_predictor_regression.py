...
def test_cli_predict_writes_outputs_for_path_outroot(monkeypatch, tmp_path):
    ...
    ref = pd.read_csv(REF_CSV)
    ref_scores = ref["fdewet (kJ/mol/water)"].to_numpy(np.float32)
    assert df.shape == (len(ref), 2)
    assert list(df.columns) == ["residue", "Fdewet"]
    np.testing.assert_array_equal(
        df["residue"].astype(str).to_numpy(),
        ref["resid"].astype(str).to_numpy(),
    )
    out_scores = df["Fdewet"].to_numpy(np.float32)
...
def test_cli_predict_parts_writes_single_structure_decomposition(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr(
        sys,
        "argv",
        ["fasthydromap", "predict", str(PDB_PATH), "-o", str(outroot), "--parts"],
    )
    ...
    np.testing.assert_allclose(
        df["Fdewet"].to_numpy(np.float32),
        df["Fdewet_intrinsic"].to_numpy(np.float32) + df["Fdewet_context"].to_numpy(np.float32),
        atol=0.02,
    )


def test_cli_predict_with_trajectory_keeps_frame_wide_csv(monkeypatch, tmp_path):
    outroot = tmp_path / "proteinG_predict"
    monkeypatch.setattr(
        sys,
        "argv",
        [
    ...
    assert "6" in df.columns


def test_cli_predict_trajectory_writes_total_output_by_default(monkeypatch, tmp_path):
    ...
    for path in (total_csv, pdb_path):
        assert path.exists()
    for path in (intrinsic_csv, context_csv, summary_csv):
...
def test_cli_predict_trajectory_parts_writes_decomposition_outputs(monkeypatch, tmp_path):
    outroot = tmp_path / "proteinG_traj_parts"
    monkeypatch.setattr(
        sys,
        "argv",
        [
...
def test_cli_predict_writes_bfactors_with_nonprotein_and_unknown_residues(monkeypatch, tmp_path):
    ...
    df = pd.read_csv(Path(f"{outroot}.csv"))
    assert df["residue"].iloc[0].startswith("A:")
    assert {"B:1", "B:2"}.issubset(set(df["residue"]))
